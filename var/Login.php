...
class Widget_Login extends Widget_Abstract_Users implements Widget_Interface_Do
{
    ...
    public function action()
    {
        ...
        /**
            极验验证TEST
         */
        require_once dirname(__FILE__) . '/../../admin/geetest/class.geetestlib.php';
        require_once dirname(__FILE__) . '/../../admin/geetest/config.php';
        session_start();
        $GtSdk = new GeetestLib(CAPTCHA_ID, PRIVATE_KEY);

        $data = array("gttest" => $_SESSION['gttest'],);
        if ($_SESSION['gtserver'] == 1) {
            $result = @$GtSdk->success_validate($_POST['geetest_challenge'], $_POST['geetest_validate'], $_POST['geetest_seccode'], $data);
            if (!$result) {
                $this->widget('Widget_Notice')->set(_t('请点击验证！'), 'error');
